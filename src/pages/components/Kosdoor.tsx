...
import { useGetKossQuery } from "../../redux/feature/kosApiSlice";

const Kosdoor = () => {
  const { data, isLoading, isSuccess } = useGetKossQuery("getKoss");
  let content;
  if (isLoading) {
    content = <Loading />;
  } else if (isSuccess) {
  ...
  return (
    <>
      <div className="flex items-center justify-center flex-col mt-7">
        <h2 className="text-4xl mb-5">Sản Phẩm Cửa Nhựa Kos</h2>
